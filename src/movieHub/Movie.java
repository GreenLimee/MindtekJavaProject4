...
public class Main {
    ...
    public static void getMoviesInYear(ArrayList<Movie> movieList, int year1, int year2) {
        ...
        for (Movie m : movieList) {
            if (m.getYear() >= year1 && m.getYear() <= year2) list.add(m);
        }
        printMovies(list);
    }

    public static void getMoviesInGenre(ArrayList<Movie> movieList, String genre) {
        for (Movie m : movieList) {
            ArrayList<Movie> list = new ArrayList<>();
            if (m.getGenre().toLowerCase().contains(genre.toLowerCase())) list.add(m);
            printMovies(list);
        }

    }

    public static void getMoviesByDirector(ArrayList<Movie> movieList, String director){
        ArrayList<Movie> list = new ArrayList<>();
        for(Movie m: movieList) if(m.getDirector().toLowerCase().trim().contains(director.toLowerCase())) list.add(m);
        printMovies(list);
    }
    public static void getMoviesInRate(ArrayList<Movie> movieList, double a, double b){
        ArrayList<Movie> list = new ArrayList<>();
        for(Movie m: movieList) if(m.getImdbRate()>=a && m.getImdbRate()<=b) list.add(m);
        printMovies(list);
    }
}
